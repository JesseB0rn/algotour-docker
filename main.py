...
def calculate_path(startpoint: GeoPoint, endpoint: GeoPoint, model = EModel.ALL):
  ...
  match model:
    case EModel.LAKES:
      cmd=f"{QGS_PRC_PATH} run model:LeastCostCorridorWalk --distance_units=meters --area_units=m2 --ellipsoid=EPSG:7004 --startp='{startpoint.longitude},{startpoint.latitude} [EPSG:4326]' --endp='{endpoint.longitude},{endpoint.latitude} [EPSG:4326]' --points_crs='EPSG:4326' --risikokarte={MODELS['water']} --dem={DEM} --avy_cost_coeff=28 --steep_downhill=5 --downhill=2.5 --flat=0.25 --uphill=2.5 --steep_uphill=5 --outputpath={OUTPATH}out_$(uuidgen).geojson --json"
    ...
    case EModel.NAIVE:
      ...
      cmd=f"{QGS_PRC_PATH} run model:LeastCostCorridorWalk --distance_units=meters --area_units=m2 --ellipsoid=EPSG:7004 --startp='{startpoint.longitude},{startpoint.latitude} [EPSG:4326]' --endp='{endpoint.longitude},{endpoint.latitude} [EPSG:4326]' --points_crs='EPSG:4326' --risikokarte={MODELS['base']} --dem={DEM} --avy_cost_coeff=28 --steep_downhill=5 --downhill=2.5 --flat=0.25 --uphill=2.5 --steep_uphill=5 --outputpath={OUTPATH}out_$(uuidgen).geojson --json"
    case _:
      ...
      cmd=f"{QGS_PRC_PATH} run model:LeastCostCorridorWalk --distance_units=meters --area_units=m2 --ellipsoid=EPSG:7004 --startp='{startpoint.longitude},{startpoint.latitude} [EPSG:4326]' --endp='{endpoint.longitude},{endpoint.latitude} [EPSG:4326]' --points_crs='EPSG:4326' --risikokarte={MODELS['bridge']} --dem={DEM} --avy_cost_coeff=28 --steep_downhill=5 --downhill=2.5 --flat=0.25 --uphill=2.5 --steep_uphill=5 --outputpath={OUTPATH}out_$(uuidgen).geojson --json"
